...
<script>
	...
	export default {
		...
		methods: {
			...
			//选中日期
			onSelectDate(date) {
				if (~this.type.indexOf('range') && this.checkeds.length == 2 && !this.disabledSelectStart) this.checkeds = [];
				else if (!(~this.type.indexOf('range')) && this.checkeds.length && !this.disabledSelectStart) this.checkeds = [];
				if(!this.disabledSelectStart){
					this.checkeds.push(new Date(date.dateObj));
					this.checkeds.sort((a, b) => a - b); //从小到大排序
				}else{
					this.checkeds.splice(1,1,new Date(date.dateObj));
				}
...
<style lang="scss" scoped>
	...
	.picker-modal-header, .picker-modal-time {
		view, text {
			color: #333
		}
	}
