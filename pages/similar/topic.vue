<template>
    <view>
        <view class="similar_nav t_center ">
			<image v-if="list.length>0&&list[0].cover" :src="$realSrc(list[0].cover)" mode="aspectFill" class="similar_img"  @tap="tolook(list[0].videoId)"></image>
            <image v-else src="/static/topic_mc.png" class="similar_img" @tap="tohome(all.info.uid)"></image>
            <view class="">
                <view class="similar_title">{{ all.info.name }}</view>
                <view class="similar_author iconfont icon-arrow-right" @tap="tohome(all.info.uid)">{{ all.info.nickname }}(首发)</view>
